class ProductsController < ApplicationController
  ...
  def index
    ...
    @products = Category.find_by(title: params[:title].capitalize).products.where(brand_id: params[:brand_id].to_i > 0 ? params[:brand_id].to_i : CategoryBrand.select(:brand_id).where(category_id: Category.find_by(title: params[:title]).id)).limit(10).offset((params[:page] - 1) * 10)
